...
namespace Toml
{
    public class Group
    {
        ...
        {
            //Console.WriteLine(entry.FullName + " : " + this.FullKey);
            var pathParts = entry.FullName.Split(Group.Separators, StringSplitOptions.None);

            //Console.WriteLine(pathParts[0] + " : " + this.FullKey);
            string[] arr = (pathParts.Take(pathParts.Length - 1).Select(s => s + ".")).ToArray<string>();
            var parentPath = string.Concat(arr);
            //Console.WriteLine(parentPath + " : " + this.FullKey);
